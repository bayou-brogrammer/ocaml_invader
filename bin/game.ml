...
type collision = { bullet : Bullet.bullet; invader : Invader.invader }
...
let invaderDirectionGuesser ~game =
  ...
  | Some _ -> (
      match game.invadersDirection with Left -> Right | Right -> Left)

let rec collectInvadersCollisions ~invaders ~bullets =
  let open Bullet in
  match invaders with
  | [] -> []
  | hd :: tl -> (
      ...
      | None -> collectInvadersCollisions ~invaders:tl ~bullets
      | Some bullet ->
          { invader = hd; bullet }
          :: collectInvadersCollisions ~invaders:tl ~bullets)

let collectInvaderBulletCollisions ~game =
  let open Bullet in
  let spaceshipBullets =
    List.filter (fun bullet -> bullet.shooter == Spaceship) game.bullets
  in
  let collisions =
    collectInvadersCollisions ~invaders:game.invaders ~bullets:spaceshipBullets
  in
  collisions

let getSpaceshipCollisionBullet ~game =
  let open Bullet in
  let invaderBullets =
    List.filter (fun bullet -> bullet.shooter == Invader) game.bullets
  in
  let checkHit =
    isInBoundsBullet ~bounds:(Spaceship.collisionBounds game.spaceship)
  in
  List.find_opt checkHit invaderBullets

let controller game = function
  | Move direction ->
      let op = match direction with Left -> ( -. ) | Right -> ( +. ) in
  ...
  | Shoot ->
      game.bullets <-
        game.bullets
        @ [
            { x = game.spaceship.x; y = game.spaceship.y; shooter = Spaceship };
          ];
      game
  | InvaderShoot ->
      if List.length game.invaders > 0 then
        ...
        game.bullets <-
          game.bullets @ [ { x = invader.x; y = invader.y; shooter = Invader } ]
      else ();
      game
  | CheckCollisions ->
      let lives =
      ...
      game.bullets <-
        List.filter
          (fun bullet ->
            ...
            &&
            match
              List.find_opt (fun col -> col.bullet == bullet) invaderCollisions
            with
            | None -> true
            | _ -> false)
          game.bullets;
      ...
      game.lives <- lives;
      game.over <- game.lives <= 0 || game.score >= 600;

      game
  | AdvanceInvaders ->
      let dir = invaderDirectionGuesser ~game in
      let forward = game.invadersDirection != dir in
      let increment = invaderIncrementResolver ~direction:dir ~forward in
      let advanceInvaders = Invader.advance ~increment in
      game.invadersDirection <- dir;
  ...
  | AdvanceBullets ->
      ...
      game.bullets <-
        List.filter
          (Bullet.isInBoundsBullet ~bounds:(50., 50., 400., 600.))
          bullets;
      game
...
let renderHome game =
  ...
  let endText =
    ...
    | _ -> Printf.sprintf "GAME OVER"
  in
  Utils.drawString ~font:Glut.BITMAP_TIMES_ROMAN_24 150. 290. endText;
  Glut.swapBuffers ()

(* Each module (Score, Life, Invader, Bullet, ...) has its own render *)
(* So we clear buffer, draw the game and swap buffers to render it to the screen *)
let renderGame game =
  GlClear.clear [ `color ];
  Score.render game.score;
  Life.render game.lives;
...
let render game =
  match game.over with false -> renderGame game | true -> renderHome game
